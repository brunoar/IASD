...
class Airplanes:
    ...
    def __init__(self, planes_list):
        for plane in planes_list:
            self.planes.append([plane[1]])
            self.classes.append([plane[2]])
            self.last_leg.append([-1])
            self.time.append([0])

    def get_class(self, plane):
        index = [i for i, x in enumerate(self.planes) if self.planes == plane]
        return self.classes[index]

    def perform_leg(self, leg, plane):
        index = [i for i, x in enumerate(self.planes) if self.planes == plane]
        last_leg[index] = leg
